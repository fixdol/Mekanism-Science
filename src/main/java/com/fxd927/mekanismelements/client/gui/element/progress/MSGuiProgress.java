package com.fxd927.mekanismelements.client.gui.element.progress;

import com.fxd927.mekanismelements.client.recipe_viewer.interfaces.IMSRecipeViewerRecipeArea;
...
public class MSGuiProgress extends GuiTexturedElement implements IMSRecipeViewerRecipeArea<MSGuiProgress>, ISupportsWarning<MSGuiProgress> {
    ...
    public MSGuiProgress(IProgressInfoHandler.IBooleanProgressInfoHandler handler, ProgressType type, IGuiWrapper gui, int x, int y) {
        this((IProgressInfoHandler) handler, type, gui, x, y);
    }

    public MSGuiProgress(IProgressInfoHandler handler, ProgressType type, IGuiWrapper gui, int x, int y) {
        super(type.getTexture(), gui, x, y, type.getWidth(), type.getHeight());
        this.type = type;
        this.handler = handler;
    }

    public MSGuiProgress colored(GuiProgress.ColorDetails colorDetails) {
        this.colorDetails = colorDetails;
        return this;
    }

    @Override
    public MSGuiProgress warning(@NotNull WarningTracker.WarningType type, @NotNull BooleanSupplier warningSupplier) {
