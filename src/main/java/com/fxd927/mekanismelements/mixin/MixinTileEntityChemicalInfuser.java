...
public abstract class MixinTileEntityChemicalInfuser extends TileEntityRecipeMachine<ChemicalChemicalToChemicalRecipe> implements
    ...
    public IChemicalTankHolder getInitialChemicalTanksRedirect(ChemicalTankHelper instance, IContentsListener listener, IContentsListener recipeCacheListener, IContentsListener recipeCacheUnpauseListener) {
        ...
        builder.addTank(leftTank = BasicChemicalTank.create(MAX_GAS, 
                (chemical, automationType) -> ChemicalTankHelper.radioactiveInputTankPredicate(() -> centerTank).test(chemical.getStack(1), automationType),
                (chemical, automationType) -> true, 
                chemical -> containsRecipe(chemical.getStack(1)), 
                ChemicalAttributeValidator.ALWAYS_ALLOW, recipeCacheListener));
        builder.addTank(rightTank = BasicChemicalTank.create(MAX_GAS, 
                (chemical, automationType) -> ChemicalTankHelper.radioactiveInputTankPredicate(() -> centerTank).test(chemical.getStack(1), automationType),
                (chemical, automationType) -> true, 
                chemical -> containsRecipe(chemical.getStack(1)), 
                ChemicalAttributeValidator.ALWAYS_ALLOW, recipeCacheListener));
