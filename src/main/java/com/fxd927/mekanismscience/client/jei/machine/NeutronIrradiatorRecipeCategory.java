...
public class NeutronIrradiatorRecipeCategory extends BaseRecipeCategory<NeutronIrradiatingRecipe> {
    private final GuiGauge<?> inputGauge;
    private final GuiGauge<?> outputGauge;
    private final GuiSlot inputSlot;

    public NeutronIrradiatorRecipeCategory(IGuiHelper helper, MekanismJEIRecipeType<NeutronIrradiatingRecipe> recipeType) {
        super(helper, recipeType, MSBlocks.NEUTRON_IRRADIATOR, 3, 3, 170, 79);
    ...
    @Override
    public void setRecipe(@NotNull IRecipeLayoutBuilder builder, NeutronIrradiatingRecipe recipe, @NotNull IFocusGroup focusGroup) {
        ...
        List<GasStack> scaledGases = gasInputs.stream().map(gas -> new GasStack(gas, gas.getAmount() * TileEntityNeutronIrradiator.BASE_TICKS_REQUIRED))
                .toList();
