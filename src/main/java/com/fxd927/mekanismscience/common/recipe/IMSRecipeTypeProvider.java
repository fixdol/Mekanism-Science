...
public interface IMSRecipeTypeProvider<VANILLA_INPUT extends RecipeInput, RECIPE extends MekanismRecipe<VANILLA_INPUT>, INPUT_CACHE extends IInputRecipeCache>{
    default ResourceLocation getRegistryName() {
        return getMSRecipeType().getRegistryName();
    }

    MSRecipeType<VANILLA_INPUT, RECIPE, INPUT_CACHE> getMSRecipeType();
    ...
    default List<RecipeHolder<RECIPE>> getRecipes(RecipeManager recipeManager, @Nullable Level world) {
        return getMSRecipeType().getRecipes(recipeManager, world);
    }
